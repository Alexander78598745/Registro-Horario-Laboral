...
class RegistroHorarioApp {
    ...
    showDemoMode() {
        // En modo demo, la aplicación siempre está disponible
    ...
    async generatePDF(data) {
        ...
        // Espacio para logo (sin banner demo ya que el PDF está perfecto)
    ...
    async sendEmailNow(emailData) {
        ...
        const emailBody = `
            ...
            <p><strong>Estado:</strong> ${emailData.fileName.includes('DEMO') ? 'Documento Demo' : 'Documento Oficial'}</p>
            <br>
    ...
    async saveRegistro(data) {
        ...
        const registros = JSON.parse(localStorage.getItem('registros_horarios_demo') || '[]');
        registros.push({ ...data, id: Date.now().toString(), demo: true });
        localStorage.setItem('registros_horarios_demo', JSON.stringify(registros));
        
        console.log('Registro DEMO guardado localmente:', data);
    }
